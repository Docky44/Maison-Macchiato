    ...
    async executeSlash(client, interaction) {
        ...
        const embedUserInfo = new EmbedBuilder()
            ...
            .addFields({
                ...
                value: `>>> **Nom :** ${inlineCode(user.tag)}\n**ID :** ${inlineCode(user.id)}\n**Création du compte :** ${createdAt.format("DD MMMM YYYY HH:mm")} (il y a ${moment.duration(now.diff(createdAt)).humanize()})\n**Bot :** ${user.bot ? "Oui" : "Non"}`,
                inline: false
            })
        ...
        if (member) {
            embedUserInfo.addFields({
                ...
                value: `>>> **A rejoint le serveur le :** ${joinedAt.format("DD MMMM YYYY HH:mm")} (il y a ${moment.duration(now.diff(joinedAt)).humanize()})\n**Rôle le plus élevé :** ${member.roles.highest}`,
                inline: false
            })
        }
