const { SlashCommandBuilder, PermissionsBitField, EmbedBuilder, messageLink, codeBlock, inlineCode, spoiler } = require("discord.js");
...
module.exports = {
    name: "server-info",
    description: "Cette commande te permet d'obtenir des informations sur le serveur............",
    categorie: "info",
    ...
    async executeSlash(client, interaction, functionJS) {
        ...
        const embedServerInfo = new EmbedBuilder()
            ...
            .setThumbnail(interaction.guild.iconURL({ dynamic: true, size: 2048, format: "png" })) //Image qui se trouve en haut à droite
            .addFields( //Permet d'ajouter un petit titre et une description en plus
                {
                    name: `📜 - Informations sur le serveur : `, //Titre du nouveau field
                    value: (`>>> **Nom :** ${inlineCode(interaction.guild.name)}\n **ID :** ${interaction.guild.id}\n **Date de création :** ${date.format("DD MMMM YYYY HH:mm")} (il y a ${duration.humanize()})\n **Propiéraire du serveur :**  ${owner}\n **ID du propiéraire :**  ${inlineCode(interaction.guild.ownerId)}`), //Text en dessous du titre
