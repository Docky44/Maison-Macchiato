...
module.exports = {
    ...
    async executeSlash(client, interaction, functionJS) {
        ...
        if (interaction.user.id === user.id) {
            await interaction.reply({ content: `Tu peux pas te bannir toi même !`,  ephemeral: true});
            return;
        }
        if (user.permissions.has(PermissionsBitField.Flags.Administrator)) {
            await interaction.reply({content: `Tu peux pas bannir une personne plus importante que toi !`,  ephemeral: true });
            return;
        }
